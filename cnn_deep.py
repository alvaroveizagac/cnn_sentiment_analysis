...
import numpy as np
import argparse
from cnn_deep_architecture import CnnDeepArch
...
parser.add_argument("--filter_sizes", help="CNN filter sizes", type=str, default="3,4,5")
parser.add_argument("--filters_strides", help="Filters and Strides", type=dict, default={"3": (3, 2), "4": (4, 1)})
...
parser.add_argument("--results_directory", help="Directory to save all the results", type=str, default="output/output_shallow")
load = ''
...
with tf.Session(config=session_conf) as sess:
    ...
    cnn = CnnDeepArch(
        ...
        embedding_size=args.embedding_size, filter_sizes=args.filter_sizes, num_filters=args.num_filters,
        l2_reg_lambda=args.l2_reg_lambda, train_size=len(x_train), batch_size=args.batch_size,
        strides_filters=args.filters_strides)
    # sess.run(init_op)
