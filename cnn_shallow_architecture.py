...
class cnn_shallow_arch(object):
    def __init__(self, words_per_document, num_classes, vocabulary_size,
                 embedding_size, filter_sizes, num_filters, l2_reg_lambda, train_size, batch_size):
        # Placeholders
        ...
        with tf.name_scope("loss"):
            cross_entropy_r = tf.nn.softmax_cross_entropy_with_logits(logits=self.scores, labels=self.y_place_holder)
        ...
        with tf.name_scope("accuracy"):
            correct_prediction = tf.equal(self.predictions, tf.argmax(self.y_place_holder, 1), name="correct_prediction")
